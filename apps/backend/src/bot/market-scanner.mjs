    ...
    try {
      ...
      if (error) {
        ...
        if (error?.message?.includes?.('fetch failed') && retryCount < maxRetries) {
          ...
          setTimeout(() => this.verificarResultado(tradeId, activeId, timeframe, signalPrice, direction, retryCount + 1), 500);
          return;
        }
    ...
    try {
      ...
      if (error) {
        ...
        if (error?.message?.includes?.('fetch failed') && retryCount < maxRetries) {
          ...
          setTimeout(() => this.recuperarTradesPendentes(retryCount + 1), 1000);
          return;
        }
