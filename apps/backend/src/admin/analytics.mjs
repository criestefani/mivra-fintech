 ...
 *
 * ✅ OPTIMIZED with Redis caching to reduce database queries by 95%
 *
 * @module admin/services/analytics
 */

import { supabase } from '../../config/supabase.mjs';
import { getOrCache, cacheClient } from '../cache/redis-client.mjs';
import { CACHE_KEYS, CACHE_TTL } from '../cache/cache-keys.mjs';

/**
 * Get complete dashboard metrics
 * Priority: REAL account metrics
 * ✅ CACHED: 6 queries → 1 query per cache period (60s)
 */
 ...
 * ✅ CACHED: 1 query per cache period (300s)
 */
export async function getRevenueData(days = 30) {
  return getOrCache(
    CACHE_KEYS.REVENUE_DATA(days),
    async () => {
 ...
 * ✅ CACHED: 1 query per cache period (600s)
 */
export async function getUserGrowthData(days = 30) {
  return getOrCache(
    CACHE_KEYS.USER_GROWTH_DATA(days),
    async () => {
      const { data, error } = await supabase.rpc('exec_sql', {
        query: `
          SELECT
...
export async function getTopUsersByPnL(limit = 10, accountType = 'real') {
  return getOrCache(
    CACHE_KEYS.TOP_USERS_BY_PNL(limit, accountType),
    async () => {
      const { data, error } = await supabase.rpc('exec_sql', {
        query: `
          SELECT
...
export async function getTopUsersByVolume(limit = 10, accountType = 'real') {
  return getOrCache(
    CACHE_KEYS.TOP_USERS_BY_VOLUME(limit, accountType),
    async () => {
      const { data, error } = await supabase.rpc('exec_sql', {
        query: `
          SELECT
...
export async function getAssetPerformance(limit = 20, accountType = 'real') {
  return getOrCache(
    CACHE_KEYS.ASSET_PERFORMANCE(limit, accountType),
    async () => {
      const { data, error } = await supabase.rpc('exec_sql', {
        query: `
          SELECT
